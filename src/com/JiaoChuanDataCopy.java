...
public class JiaoChuanDataCopy {
	
	static boolean  CopyData(String suoshu,String xinghaoID,String table,String yuanjieduan,String xianjieduan)
	{
		
		System.out.println("^^^^^^^^^^^"+suoshu+"^^^^^^^^^^^^^^^^^^^^^^^");
		System.out.println("^^^^^^^^^^^"+xinghaoID+"^^^^^^^^^^^^^^^^^^^^^^^");
		System.out.println("^^^^^^^^^^^"+yuanjieduan+xianjieduan+"^^^^^^^^^^^^^^^^^^^^^^^");
		List cols=new ArrayList();
		cols.add("SUOSHU");
		cols.add("SUOSHUJIEDUAN");
		List values=new ArrayList(); 
		values.add(xinghaoID);
